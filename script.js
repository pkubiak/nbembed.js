...
function loadJupyter(url, iframe, config) {
	...
	$.ajax({
		...
		send({type: 'render', data: content, config: config});
	});
}
...
$(function(){
	$('notebook').each(function(i, el) {
		...
		el.insertAdjacentHTML('beforeend', '<div style="text-align:right;font-size:12px;opacity:0.6;padding-right:20px">Rendered with <a href="#">confluence-notebook</a></div>');
